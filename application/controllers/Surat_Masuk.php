...
class Surat_Masuk extends CI_Controller
{
    ...
    public function tambah_surat()
    {
        ...
        if (($this->form_validation->run() == TRUE) && (!empty($_FILES['file']['name']))) {
            ...
            $this->session->set_flashdata('notification_gagal', 'Surat Masuk gagal ditambahkan, cek inputan anda');
            redirect('Surat_Masuk');
        }
    }
    ...
    public function edit_surat()
    {
        ...
        if (($this->form_validation->run() == TRUE)) {
            ...
            $this->session->set_flashdata('notification_gagal', 'Data Surat Masuk gagal diedit, cek inputan anda');
            redirect('Surat_Masuk');
        }
    }
    ...
    public function delete_surat()
    {
        ...
        //load data folder
        $data['surat_masuk'] = $this->Surat_Masuk_Model->select_data_surat_byId($id)->row();
        $file = $data['surat_masuk']->file;

        //hapus file dari folder
        $filehapus = './assets/upload/surat_masuk/' . $file;
        unlink($filehapus);

        //hapus data repositori
        $this->Surat_Masuk_Model->delete_surat($id);
    }
}
